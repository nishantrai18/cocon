...
class ResNet2d3d_full(nn.Module):
    def __init__(self, block, layers, track_running_stats=True, in_channels=3):
        super(ResNet2d3d_full, self).__init__()
        self.inplanes = 64
        self.track_running_stats = track_running_stats
        bias = False
        self.conv1 = nn.Conv3d(in_channels, 64, kernel_size=(1,7,7), stride=(1, 2, 2), padding=(0, 3, 3), bias=bias)
...
def neq_load_customized(model, pretrained_dict):
    ...
    for k, v in pretrained_dict.items():
        if k in model_dict:
            ...
            names.append(k)
    print(set([k.split('.')[-1] for k in names]))
    print('---------------------------')
