...
class ScaleForTensors:
    ...
    def resize_multi_channel_image(self, img_tensor_list, size):
        c, h, w = img_tensor_list[0].shape
        assert c < 20, "Invalid shape: {}".format(img_tensor_list.shape)

        resized_channels = [
            torch.stack([
                self.toTensor(self.toPILImage(img_tensor_list[idx][c]).resize(size, self.interpolation)).squeeze(0)
                    for c in range(img_tensor_list[idx].shape[0])
            ]) for idx in range(len(img_tensor_list))
        ]
        resized_img_tensor = torch.stack(resized_channels)
        assert resized_img_tensor[0].shape == (c, size[0], size[1]), \
            "Invalid shape: {}, orig: {}".format(resized_img_tensor.shape, img_tensor_list[0].shape)
        return resized_img_tensor

    def __call__(self, img_tensor_list):
        # assert len(imgmap) > 1 # list of images
        img1 = img_tensor_list[0]
        if isinstance(self.size, int):
            c, h, w = img1.shape
            if (w <= h and w == self.size) or (h <= w and h == self.size):
            ...
            if w < h:
                ow = self.size
                oh = int(self.size * h / w)
                return self.resize_multi_channel_image(img_tensor_list, (ow, oh))
            else:
                oh = self.size
                ow = int(self.size * w / h)
                return self.resize_multi_channel_image(img_tensor_list, (ow, oh))
        else:
            return self.resize_multi_channel_image(img_tensor_list, self.size)
...
class RandomIntensityCropForTensors:
    ...
    def __call__(self, img_tensor_list):
        ...
        # Expected format
        c, h, w = img1.shape
        assert c < 20, "Invalid channel size: {}".format(img1.shape)
        ...
        if self.size is not None:
            ...
            proposals = []
            # number of proposal: use the one with largest sum of values
            for j in range(3):
                x = random.randint(0, w - tw)
                y = random.randint(0, h - th)
                val = \
                    sum([torch.mean(torch.abs(img_tensor_list[idx][:, y:y + th, x:x + tw])) for idx in range(len(img_tensor_list))])
                proposals.append(((x, y), val))

            (x, y), _ = max(proposals, key=lambda x: x[1])
            crops = [i[:, y:y + th, x:x + tw] for i in img_tensor_list]
            return crops
        else:
            return img_tensor_list


class RandomIntensityCropForFlow:
    def __init__(self, size):
        if isinstance(size, numbers.Number):
            self.size = (int(size), int(size))
        else:
    ...
    def __call__(self, imgmap):
        ...
        if self.size is not None:
            ...
            # Process img_arrs
            img_arrs = [np.asarray(img, dtype=float) for img in imgmap]
            img_arrs = [(img * 0.0) + 127. if np.max(img) < 10.0 else img for img in img_arrs]
            # Assuming that flow data passed has mean > 100.0
            img_arrs = np.array(img_arrs) - 127.
            ...
            for j in range(3):
                try:
                    ...
                    print("Error:", w, h, tw, th, img_arrs.shape)
                val = np.mean(np.abs(img_arrs[:, y:y + th, x:x + tw, :]))
                proposals.append(((x, y), val))

            (x, y), _ = max(proposals, key=lambda x: x[1])
            crops = [i.crop((x, y, x + tw, y + th)) for i in imgmap]
            return crops
        else:
            return imgmap


class CenterCropForTensors:
    def __init__(self, size):
        if isinstance(size, numbers.Number):
            self.size = (int(size), int(size))
        else:
    ...
    def __call__(self, img_tensor_list):
        ...
        y = int(round((h - th) / 2.))
        try:
            result = [img_tensor[:, y:y + th, x:x + tw] for img_tensor in img_tensor_list]
        except:
            print(img_tensor_list[0].shape, y, th, x, tw)
        return result
